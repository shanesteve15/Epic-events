...
@Composable
fun ProductListScreen(navController: NavController, users: List<Product>) {
    ...
    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = {
                    Text(text = "users",fontSize = 30.sp, color = Color.White)
                },
                navigationIcon = {
                    IconButton(onClick = {
...
private suspend fun fetchusers(onSuccess: (List<Product>) -> Unit) {
...
suspend fun fetchProduct(productId: String, onSuccess: (Product?) -> Unit) {
    val firestore = Firebase.firestore
    val docRef = firestore.collection("users").document(productId)
